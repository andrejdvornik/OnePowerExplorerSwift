...
class StorageUser():
	...
	def getStoredObject(self, object_name, interpolator = False, inverse = False, path = None,
					store_interpolator = True, store_path_data = True, linear_interp = False):
		"""
		...
		linear_interp: bool
			If ``True``, we fall back to linear (rather than cubic spline) interpolation. This
			can be used to avoid spline ringing, but it is much less accurate in general.
