...
class Cosmology(object):
	...
	def __init__(self, name = None,
		...
		# Similarly, we sometimes need to fall back to linear interpolation, in which case we 
		# use many more bins.
		self.R_min_sigma = 1E-12
		self.R_max_sigma = 1E3
		self.R_Nbins_sigma = 18.0
		self.R_Nbins_sigma_klimits = 60.0
		self.R_Nbins_sigma_linear = 200.0
	...
	def _matterPowerSpectrumLimits(self, model = defaults.POWER_SPECTRUM_MODEL, **ps_args):
		...
		if (model in power_spectrum.models):
		...
		elif ('path' in ps_args) and (ps_args['path'] is not None):
		...
		elif ('func' in ps_args) and (ps_args['func'] is not None):
			if 'kmin' in ps_args:
				k_min = ps_args['kmin']
			if 'kmax' in ps_args:
				k_max = ps_args['kmax']
			...
			raise Exception('Power spectrum model is not self-similar, not a known model, and does not have a path to a file or function.')
	...
	def _matterPowerSpectrumExact(self, k, model = defaults.POWER_SPECTRUM_MODEL, 
		...
		if self.power_law:
			...
			Pk = ps_args['func'](k, model, self, **ps_args)
	...
	def _matterPowerSpectrumInterpolator(self, model = defaults.POWER_SPECTRUM_MODEL, 
		...
		if interpolator is None:
			if (not 'path' in ps_args) or (ps_args['path'] is None):
				...
				if np.any(data_Pk <= 0.0):
					mask = data_Pk > 0.0
					data_k = data_k[mask]
					data_Pk = data_Pk[mask]
	...
	def matterPowerSpectrum(self, k, z = 0.0, model = defaults.POWER_SPECTRUM_MODEL,
		...
		reading (though not necessarily writing) of files. Similarly, the user can provide a 
		function pointer.

		Warnings
		--------
		When providing a table or function for the power spectrum, please make sure to choose a 
		unique ``model`` name. Even if persistence is off (``persistence = ''``), that does NOT
		disable storage within memory, which is accessed via a string generated from the ``model``
		parameter. Thus, subsequent evaluations of different models with the same ``model`` name
		can lead to errors.
		...
		ps_args: kwargs
			...
			:math:`({\\rm Mpc}/h)^3`). The limits in :math:`k`-space are taken to be the lower and
			upper limits of the table. Alternatively, the user can also specify a ``func`` 
			parameter that contains a function, which must have the signature 
			``func(k, model, cosmo, **ps_args)``. The input and output units are the same as for 
			tables. In this case, limits in :math:`k`-space can optionally be provided by adding 
			``kmin`` and/or ``kmax`` parameters to ``ps_args``.
	...
	#
	# Note that the table for the forward and backward interpolators are the same, so we increase
	# bins even for the forward interpolation table if the inverse table is linear and thus needs
	# more bins.

	def _sigmaInterpolator(self, j, filt, inverse, kmin, kmax, linear_inverse, ps_args):
		...
		interp_args = dict(interpolator = True, inverse = inverse, linear_interp = (inverse and linear_inverse))
		interpolator = self.storageUser.getStoredObject(table_name, **interp_args)
		...
		if interpolator is None:
			...
			if linear_inverse:
				n_bins = self.R_Nbins_sigma_linear
			elif (kmin is not None) or (kmax is not None):
	...
	def sigma(self, R, z = 0.0, j = 0, filt = 'tophat', inverse = False, derivative = False, 
							kmin = None, kmax = None, linear_inverse = False, ps_args = defaults.PS_ARGS):
		"""
		...
		kmax: float 
			...
			cutoff scale.
		linear_inverse: bool
			If True, the inverse interpolation of :math:`R(\\sigma)` is performed with a linear 
			rather than a spline interpolator. This is slower and/or less accurate, but it can be
			necessary if :math:`\\sigma(R)` approaches a constant, e.g., because of a power 
			spectrum cutoff.
		ps_args: dict
		...
		if self.interpolation:
			interpolator = self._sigmaInterpolator(j, filt, inverse, kmin, kmax, linear_inverse, ps_args)
			...
			if not inverse:
				...
				if derivative: 
					...
					if np.max(np.abs(ret)) > 50.0:
						if not linear_inverse:
							raise Exception('Found extreme values in R(sigma) interpolation. This can happen when sigma(R) approaches a constant. Please switch to linear interpolation.')
						else:
							raise Exception('Found extreme values in R(sigma) interpolation. Please check sigma(R) for your power spectrum.')
					ret = 10**ret
