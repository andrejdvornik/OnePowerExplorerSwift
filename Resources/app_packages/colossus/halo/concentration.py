...
def modelLudlow16(M200c, z, ps_args = defaults.PS_ARGS, sigma_args = defaults.SIGMA_ARGS):
	"""
	...
	sigma2_fM = cosmo.sigma(peaks.lagrangianR(f * M200c), 0.0, ps_args = ps_args, **sigma_args)**2
	sigma2_M = cosmo.sigma(peaks.lagrangianR(M200c), 0.0, ps_args = ps_args, **sigma_args)**2
...
def _diemer19_general(M, z, params, ps_args = defaults.PS_ARGS, sigma_args = defaults.SIGMA_ARGS):
	...
	n_eff = peaks.powerSpectrumSlopeFromM(M, z, slope_type = 'sigma', scale = params['kappa'], 
									ps_args = ps_args, sigma_args = sigma_args)
	alpha_eff = _diemer19_alpha_eff(z)

	nu = peaks.peakHeight(M, z, ps_args = ps_args, sigma_args = sigma_args)
...
def modelDiemer19(M200c, z, statistic = 'median', ps_args = defaults.PS_ARGS, sigma_args = defaults.SIGMA_ARGS):
	"""
	...
	return _diemer19_general(M200c, z, params, ps_args = ps_args, sigma_args = sigma_args)
